...
| `recommendations` | `id`, `report_id`, stable Shopify refs, normalized style rationale, optional generated asset, optional report-local outfit group/title/position | Store references/rationale/grouping only; refresh product facts live before display/action |
...
| `live_sessions` | `id`, `profile_id`, selected recommendation/ref, status, camera/microphone/visual consent refs, current result-set ID, action sequence, latency summaries | No provider credential/session ID, media, transcript, product payload, or raw action is persisted |
...
Uploads use one-path server-created signed upload tokens after application ownership checks; customer reads use metadata-backed authenticated policy or short-lived signed download URLs. Storage paths are opaque references in Postgres. Never place an email address, name, or catalog title in a path.
