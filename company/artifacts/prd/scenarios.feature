Feature: Magic Mirror style intelligence and shopping journey
  ...
  Scenario: A first-time customer receives a personalized style report
    ...
    And includes generated wardrobe previews linked to current real products when quality, consent, and product-image reuse permission pass, otherwise complete text and linked product recommendations
  ...
  Scenario: A returning customer uses Google or a magic link to resume the correct account state
    ...
    And a completed customer can start live styling directly from Style Home alongside the report and suggested outfits
