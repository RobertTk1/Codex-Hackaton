...
1. Execute exactly one next ticket: `ENG-016 — Create favorite-brand and garment-type size records`; preserve separate brand + garment-type evidence, bounded ordering, draft-only mutation, and owner isolation while limiting work to its migration and focused database proof.
