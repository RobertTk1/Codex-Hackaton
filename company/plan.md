...
1. Execute exactly one next ticket: `ENG-018 — Enforce profile child ownership and draft-only writes`; enforce same-owner child records, draft-only customer mutation, and fail-closed authorization while limiting work to the ticket's named profile-child contract and focused database proof.
