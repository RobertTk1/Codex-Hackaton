...
1. Execute exactly one next ticket: `ENG-017 — Create append-only consent records`; preserve purpose-specific immutable consent history, latest-decision queries, owner isolation, and the approved no-update/no-delete customer contract while limiting work to its migration and focused database proof.
