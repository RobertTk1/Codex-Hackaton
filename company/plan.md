...
1. Execute exactly one next ticket: `ENG-031 — Create bounded report notification delivery records`; persist only bounded delivery state and safe provider references without copying email addresses or report content.
...
- 2026-07-22: Complete `ENG-030` with permanent-account owner reads across report runs, immutable reports, canonical sections, and recommendations plus current-owner/unexpired reads for generated assets and source lineage. A still-anonymous owner may read current onboarding assets but receives no permanent report graph; another account and expired asset paths receive no metadata. Keep the account-deletion access-block helper and policy-wide deny predicate with its approved owner, `ENG-035`, rather than introducing it out of sequence. Focused 13-assertion, aggregate 718-assertion, signed-storage, report-concurrency, intentional-leak, 105-test application, strict check, three-build, clean-checkout, ticket/integration CI, fail-hard hosted rollback-only, advisor, development deployment, application rollback, and restoration checks pass with zero retained fixtures; production remains untouched.
- 2026-07-22: Complete `ENG-029` with one server-only, security-invoker report-publication transaction. Require exactly three canonical report sections and one-to-twenty-four normalized recommendations before writing; then atomically publish the immutable report graph, succeed the exact finalizing run, activate the submitted profile, archive the prior active profile, and enqueue one notification without delivering it. Replaying a succeeded run returns the same report, missing sections roll back every write, and concurrent calls converge on one active version. Focused 16-assertion, aggregate 705-assertion, true two-process concurrency, 105-test application, strict check, three-build, clean-checkout, ticket/integration CI, rollback-only hosted, advisor, development deployment, application rollback, and restoration checks pass with zero retained fixtures; production remains untouched.
