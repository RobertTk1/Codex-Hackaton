...
1. Execute exactly one next ticket: `ENG-019 — Create anonymous-transfer records and atomic ownership function`; implement the private transfer-token record and one-use atomic full-graph ownership transfer while limiting work to the ticket's named transfer contract and focused database proof.
